...
-- 最后修改人：
-------------------------------------------------------------------------------------------------------
EquipIconListCell = class("EquipIconListCellClass", Window)
EquipIconListCell.__size = nil
   
function EquipIconListCell:ctor()
    self.super:ctor()
	self.roots = {}
	self.equip_instance = nil
	self.isshow = false
    local function init_equip_icon_list_cell_terminal()

        state_machine.init()
    end
    
    init_equip_icon_list_cell_terminal()
end

function EquipIconListCell:onUpdateDraw()
	...
	local picIndex = dms.int(dms["equipment_mould"], self.equip_instance.user_equiment_template, equipment_mould.pic_index)
	local quality = dms.int(dms["equipment_mould"], self.equip_instance.user_equiment_template, equipment_mould.grow_level) + 1
	...
	if tonumber(self.equip_instance.ship_id) ~= 0 then
	...
	local str = "lv "..self.equip_instance.user_equiment_grade
	textlv:setString("")
	textlv:setVisible(false)
	if tonumber(self.equip_instance.user_equiment_grade) > 0 then
		textlv:setString(str)
		textlv:setVisible(true)
	end
end

function EquipIconListCell:onEnterTransitionFinish()

end

function EquipIconListCell:setSelected(isshow)
	local root = self.roots[1]
	if root == nil or self.roots == nil then
		return
	end
...
function EquipIconListCell:onExit()
...
function EquipIconListCell:init(equip_instance, index)
	self.equip_instance = equip_instance
	if index ~= nil and index < 7 then
		self:onInit()
	end

	self:setContentSize(EquipIconListCell.__size)
	return self
end

function EquipIconListCell:reload()
	local root = self.roots[1]
	if root ~= nil then
		return
	end
...
function EquipIconListCell:unload()
	local root = self.roots[1]
	if root == nil then
		return
	end
...
function EquipIconListCell:createCell()
	local cell = EquipIconListCell:new()
