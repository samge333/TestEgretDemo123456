...
-- 最后修改人：
-------------------------------------------------------------------------------------------------------
FormationEquipmentCell = class("FormationEquipmentCellClass", Window)

function FormationEquipmentCell:ctor()
    self.super:ctor()
	self.roots = {}
	self.item = nil
	self.type = 0 --物品类型：1道具，2装备，3船只

	local function init_formation_car_equip_cell_terminal()
	
		local call_formation_car_equip_cell_terminal = {
            _name = "call_formation_car_equip_cell",
		...
		state_machine.add(call_formation_car_equip_cell_terminal)	
        state_machine.init()
	end
	init_formation_car_equip_cell_terminal()
	
end

function FormationEquipmentCell:onUpdateDraw()
	...
	local touchImage_2 = fwin:addTouchEventListener(Panel_prop, nil, {terminal_name = "call_formation_car_equip_cell", terminal_state = 0, ship = self.item}, nil, 0)

end

function FormationEquipmentCell:onEnterTransitionFinish(equipmentInstance)
	local root = nil
...
function FormationEquipmentCell:clearUIInfo( ... )
	if __lua_project_id == __lua_project_l_digital
		or __lua_project_id == __lua_project_l_pokemon or __lua_project_id == __lua_project_l_naruto
		then
		local root = self.roots[1]
...
function FormationEquipmentCell:onExit()
	if __lua_project_id == __lua_project_l_digital
		or __lua_project_id == __lua_project_l_pokemon or __lua_project_id == __lua_project_l_naruto
		then
		local root = self.roots[1]
...
function FormationEquipmentCell:init(item,objectType,parameterSet)
	self.item = item
	self.type = objectType
	self.parameter = parameterSet
end

function FormationEquipmentCell:createCell()
	local cell = FormationEquipmentCell:new()
