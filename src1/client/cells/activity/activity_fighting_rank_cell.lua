...
-- 最后修改人：
-------------------------------------------------------------------------------------------------------
ActivityFightingRankCell = class("ActivityFightingRankCellClass", Window)
ActivityFightingRankCell.__size = nil
-- ActivityFightingRankCell
function ActivityFightingRankCell:ctor()
    self.super:ctor()
	self.roots = {}
    ...
    -- Initialize ActivityFightingRankCell state machine.
    local function init_ActivityFightingRankCell_terminal()
		local activity_fighting_rank_cell_draw_fund_reward_terminal = {
            _name = "activity_fighting_rank_cell_draw_fund_reward",
		...
		state_machine.add(activity_fighting_rank_cell_draw_fund_reward_terminal)
		state_machine.init()
    end
    
    -- -- call func init ActivityFightingRankCell state machine.
    init_ActivityFightingRankCell_terminal()
end

function ActivityFightingRankCell:onUpdateDrawRecharge()

end

function ActivityFightingRankCell:rewadDraw(_index)
...
function ActivityFightingRankCell:onUpdateDraw()
	local root = self.roots[1]
	local activity = _ED.active_activity[49]
	...
	-- local ImageYetGet = ccui.Helper:seekWidgetByName(root, "Image_ls_5")	--领取
	local signInButton = ccui.Helper:seekWidgetByName(root, "Text_name")	--  名字
	-- rankNum:setString(self.example.user_fighting)
	...
	local thelast = activity.activity_Info[tonumber(activity.activity_count)]  ---最后一个的区间
	local qujian1 = zstring.split(thelast.activityInfo_pai_ming, "-")
	local mam1 = tonumber(qujian1[2])
	if self.index <= mam1 then
		for i, v in pairs(activity.activity_Info) do
			local qujian = zstring.split(v.activityInfo_pai_ming, "-")
			local sam = tonumber(qujian[1])
			local mam = tonumber(qujian[2])
			
			if self.index >= sam and self.index <= mam then
				rewadArry = activity.activity_Info[i]
				drawicon(rewadArry,ListViewDraw)
			end	
		end	
	else
		rewadArry = activity.activity_Info[tonumber(activity.activity_count)]
		drawicon(rewadArry,ListViewDraw)
	end
end

function ActivityFightingRankCell:onHeadDraw(person)
...
--道具
function ActivityFightingRankCell:getItemCell(mid,mtype,count,isCertainly)
...
function ActivityFightingRankCell:onEnterTransitionFinish()

end

function ActivityFightingRankCell:onInit()

 	local root = cacher.createUIRef("activity/wonderful/activity_zhanlibang_list.csb", "root")
 	table.insert(self.roots, root)
 	self:addChild(root)
	if ActivityFightingRankCell.__size == nil then
		ActivityFightingRankCell.__size = root:getChildByName("Panel_land_list"):getContentSize()
	end	
    ...
    {
        terminal_name = "activity_fighting_rank_cell_draw_fund_reward",
...
function ActivityFightingRank:clearUIInfo( ... )
...
function ActivityFightingRankCell:onExit()
    self:clearUIInfo()
    cacher.freeRef("icon/item.csb", self.roots[2])
	cacher.freeRef("activity/wonderful/activity_zhanlibang_list.csb", self.roots[1])
end

function ActivityFightingRankCell:init(example,index,activityIndex)
	...
	self:setContentSize(ActivityFightingRankCell.__size)
end

function ActivityFightingRankCell:reload()
...
function ActivityFightingRankCell:unload()
	local root = self.roots[1]
	if root == nil then
		return
	end
	self:clearUIInfo()
	cacher.freeRef("icon/item.csb", self.roots[2])
	cacher.freeRef("activity/wonderful/activity_zhanlibang_list.csb", root)
...
function ActivityFightingRankCell:createCell()
	local cell = ActivityFightingRankCell:new()
