...
-- 最后修改人：
-------------------------------------------------------------------------------------------------------
ActivityTopRankCell = class("ActivityTopRankCellClass", Window)
ActivityTopRankCell.__size = nil
-- ActivityTopRankCell
function ActivityTopRankCell:ctor()
    self.super:ctor()
	self.roots = {}
    ...
    -- Initialize ActivityTopRankCell state machine.
    local function init_activity_top_rank_cell_terminal()
		local activity_top_rank_cell_draw_fund_reward_terminal = {
            _name = "activity_top_rank_cell_draw_fund_reward",
		...
		state_machine.add(activity_top_rank_cell_draw_fund_reward_terminal)
		state_machine.init()
    end
    
    -- -- call func init ActivityTopRankCell state machine.
    init_activity_top_rank_cell_terminal()
end

function ActivityTopRankCell:onUpdateDrawRecharge()

end

function ActivityTopRankCell:rewadDraw(_index)
...
function ActivityTopRankCell:onUpdateValueDescribe()
	local root = self.roots[1]
	local Text_12 = ccui.Helper:seekWidgetByName(root, "Text_12")
	local index = self.activityIndex 
	Text_12:setString(_top_rank_info[13+index] .. ":")
end

function ActivityTopRankCell:drawicon(rewadArry,ListViewDraw1)
	local root = self.roots[1]
	...
	if self.example ~= nil and tonumber(self.example.user_id) == tonumber(_ED.user_info.user_id) then
...
function ActivityTopRankCell:onUpdateDraw()
	local root = self.roots[1]
	local activity = _ED.active_activity[78]
	local rewadArry = nil
	self.activityId = activity.activity_id
	self.rewardIndex = (self.activityIndex - 1) * 10 + self.index
	...
	local order = zstring.tonumber(self.index)
	if order == 1 then
	...
	local Image_head = ccui.Helper:seekWidgetByName(root, "Image_head")
	...
	local Text_name = ccui.Helper:seekWidgetByName(root, "Text_name")	--  名字
	rankNum:setString("")
	Text_name:setString("")
	if self.activityDay >= self.activityIndex  then 
		self:drawicon(rewadArry,ListViewDraw)
		if self.example ~= nil then
			Panel_3_ro:addChild(self:onHeadDraw(self.example))
			if zstring.tonumber( self.example.user_fighting) > 1000000 then
				rankNum:setString(""..math.floor(zstring.tonumber( self.example.user_fighting)/10000))
			else
			...
			Text_name:setString(self.example.user_name)
		else
	...
	self:onUpdateValueDescribe()
end

function ActivityTopRankCell:onHeadDraw(person)
...
--道具
function ActivityTopRankCell:getItemCell(mid,mtype,count,isCertainly)
...
function ActivityTopRankCell:onEnterTransitionFinish()

end

function ActivityTopRankCell:onInit()

 	local root = cacher.createUIRef("activity/wonderful/activity_top_rank_list.csb", "root")
 	table.insert(self.roots, root)
 	self:addChild(root)
	if ActivityTopRankCell.__size == nil then
		ActivityTopRankCell.__size = root:getChildByName("Panel_land_list"):getContentSize()
	end	
    ...
    {
        terminal_name = "activity_top_rank_cell_draw_fund_reward",
...
function ActivityTopRankCell:clearUIInfo( ... )
...
function ActivityTopRankCell:init(example,index,activityIndex,activityDay)
	...
	--if index < 5 then
		self:onInit()
	--end
	self:setContentSize(ActivityTopRankCell.__size)
end

function ActivityTopRankCell:reload()
...
function ActivityTopRankCell:unload()
	local root = self.roots[1]
	if root == nil then
		return
	end
...
function ActivityTopRankCell:createCell()
	local cell = ActivityTopRankCell:new()
