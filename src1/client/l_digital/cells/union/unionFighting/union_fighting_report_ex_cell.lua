...
--  工会战战报cell
-- ------------------------------------------------------------------------------------------------------------
UnionFightingReportEXCell = class("UnionFightingReportEXCellClass", Window)
UnionFightingReportEXCell.__size = nil

--创建cell
local union_fighting_report_ex_cell_create_terminal = {
    _name = "union_fighting_report_ex_cell_create",
    ...
    _invoke = function(terminal, instance, params)
        local cell = UnionFightingReportEXCell:new()
        local index = params[1]
        local info = params[2]
        local count = params[3]
        local nType = params[4]
        cell:init(index, info, count, nType)
...
state_machine.add(union_fighting_report_ex_cell_create_terminal)
state_machine.init()

function UnionFightingReportEXCell:ctor()
    self.super:ctor()
    self.roots = {}
    self.actions = {}
    ...
    local function init_union_fighting_report_ex_cell_terminal()
        local union_fighting_report_ex_cell_enter_fight_terminal = {
            _name = "union_fighting_report_ex_cell_enter_fight",
            _init = function (terminal)
        ...
        local union_fighting_report_ex_cell_look_infor_terminal = {
            _name = "union_fighting_report_ex_cell_look_infor",
            _init = function (terminal)
        ...
        state_machine.add(union_fighting_report_ex_cell_enter_fight_terminal)
        state_machine.add(union_fighting_report_ex_cell_look_infor_terminal)
    end
    
    init_union_fighting_report_ex_cell_terminal()
end

function UnionFightingReportEXCell:updateUserInfo(index, info, result)
...
function UnionFightingReportEXCell:updateDraw()
    local root = self.roots[1]
...
-- 胜负,战报id@id,昵称,公会名,头像,等级,队伍,当前血量,最大血量,连胜数@昵称,公会名,头像,等级,队伍,当前血量,最大血量,连胜数|
    ...
    if leftInfo[3] == _ED.union.union_info.union_name or self._type == 2 then
...
function UnionFightingReportEXCell:onInit(isNewReport)
    local root = cacher.createUIRef(string.format(config_csb.union_fight.sm_legion_ghz_fighting_cell, 3), "root")
    table.insert(self.roots, root)
    self:addChild(root)

    fwin:addTouchEventListener(ccui.Helper:seekWidgetByName(root, "Button_vs_chakan"), nil, 
    {
        terminal_name = "union_fighting_report_ex_cell_enter_fight", 
        terminal_state = 0,
        cell = self,
    }, nil, 1)
    ...
    {
        terminal_name = "union_fighting_report_ex_cell_look_infor", 
        terminal_state = 0,
        cell = self,
        m_type = 1,
    }, nil, 1)
    ...
    if UnionFightingReportEXCell.__size == nil then
        UnionFightingReportEXCell.__size = root:getContentSize()
    end
end

function UnionFightingReportEXCell:onEnterTransitionFinish()

end

function UnionFightingReportEXCell:reload()
    local root = self.roots[1]
    if root ~= nil then
        return
    end
...
function UnionFightingReportEXCell:unload()
    local root = self.roots[1]
    if root == nil then
        return
    end
    self:clearUIInfo()
    cacher.freeRef(string.format(config_csb.union_fight.sm_legion_ghz_fighting_cell, 3), root)
    root:removeFromParent(false)
    self.roots = {}
    self.actions = {}
end

function UnionFightingReportEXCell:clearUIInfo( ... )
    if true then
        return
    end
    local root = self.roots[1]
...
function UnionFightingReportEXCell:init(index, info, count, nType)
    self._index = index 
    ...
    self:setContentSize(UnionFightingReportEXCell.__size)
    return self
end

function UnionFightingReportEXCell:onExit()
    self:clearUIInfo()
    local root = self.roots[1]
    cacher.freeRef(string.format(config_csb.union_fight.sm_legion_ghz_fighting_cell, 3), root)
end
