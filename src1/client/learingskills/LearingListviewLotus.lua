...
LearingListviewLotus = class("LearingListviewLotusClass", Window)
app.load("client.learingskills.cells.learing_list_cell")
function LearingListviewLotus:ctor()
    self.super:ctor()
    self.roots = {}
    self.list_view = nil
    ...
    local function init_learing_skills_list_lotus_terminal()
        local learing_skills_list_lotus_shot_terminal = {
            _name = "learing_skills_list_lotus_shot",
        ...
        local learing_skills_list_lotus_update_terminal = {
            _name = "learing_skills_list_lotus_update",
            _init = function (terminal)
            ...
            _state = 0,
            _invoke = function(terminal, instance, params)
                state_machine.excute("learing_skills_list_lotus_shot",0,"")
        ...
        state_machine.add(learing_skills_list_lotus_shot_terminal)
        state_machine.add(learing_skills_list_lotus_update_terminal)
        state_machine.init()
    end
    
    -- call func init hom state machine.
    init_learing_skills_list_lotus_terminal()
end
function LearingListviewLotus:getSort()
    local result = {}
    ...
    for i=1,#dms["skill_equipment_mould"] do
        ...
        if camp_preference == 3 and can_learn_skilltype == skill_type then
...
function LearingListviewLotus:onInit()
...
function LearingListviewLotus:onUpdate()
...
function LearingListviewLotus:onUpdateDraw()
...
function LearingListviewLotus:init()
    self:onInit()
end
function LearingListviewLotus:onEnterTransitionFinish()

end

function LearingListviewLotus:onExit()
    state_machine.remove("learing_skills_list_lotus_shot") 
    state_machine.remove("learing_skills_list_lotus_update")
end