...
LearingListviewDragons = class("LearingListviewDragonsClass", Window)
app.load("client.learingskills.cells.learing_list_cell")
function LearingListviewDragons:ctor()
    self.super:ctor()
	self.roots = {}
    self.list_view = nil
    ...
    local function init_learing_skills_list_dragons_terminal()
        local learing_skills_list_dragons_shot_terminal = {
            _name = "learing_skills_list_dragons_shot",
        ...
        local learing_skills_list_dragons_update_terminal = {
            _name = "learing_skills_list_dragons_update",
            _init = function (terminal)
            ...
            _state = 0,
            _invoke = function(terminal, instance, params)
                state_machine.excute("learing_skills_list_dragons_shot",0,"")
        ...
        state_machine.add(learing_skills_list_dragons_shot_terminal)
        state_machine.add(learing_skills_list_dragons_update_terminal)

        state_machine.init()
    end
    
    -- call func init hom state machine.
    init_learing_skills_list_dragons_terminal()
end
function LearingListviewDragons:getSort()
    local result = {}
    ...
    for i=1,#dms["skill_equipment_mould"] do
        ...
        if camp_preference == 1 and can_learn_skilltype == skill_type then
...
function LearingListviewDragons:onInit()
...
function LearingListviewDragons:onUpdate()
...
function LearingListviewDragons:onUpdateDraw()
...
function LearingListviewDragons:init()
	self:onInit()
end
function LearingListviewDragons:onEnterTransitionFinish()

end

function LearingListviewDragons:onExit()
    state_machine.remove("learing_skills_list_dragons_shot") 
    state_machine.remove("learing_skills_list_dragons_update")
end