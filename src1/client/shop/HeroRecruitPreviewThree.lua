...
HeroRecruitPreviewThree = class("HeroRecruitPreviewThreeClass", Window)
    
function HeroRecruitPreviewThree:ctor()
    self.super:ctor()
	self.roots = {}
	self.types  = nil
...
function HeroRecruitPreviewThree:onHeadDraw(id)
	local csbItem = nil 
...
function HeroRecruitPreviewThree:onUpdateDraw()
	...
	local last = {}
	local lastAll = {}
	local arr1 = dms.searchs(dms["bounty_hero_param"], bounty_hero_param.bounty_group, 13)
	local arr2 = dms.searchs(dms["bounty_hero_param"], bounty_hero_param.bounty_group, 14)
	local arr3 = dms.searchs(dms["bounty_hero_param"], bounty_hero_param.bounty_group, 15)
	local arr4 = dms.searchs(dms["bounty_hero_param"], bounty_hero_param.bounty_group, 16)
	...
	-- [[绘制滚动层
	local m_ScrollView = ccui.Helper:seekWidgetByName(root, "ScrollView_baiying_1")
...
function HeroRecruitPreviewThree:onEnterTransitionFinish()
	-- [[-------------------------------------
	-- add map for trial tower
    local csbHeroRecruitPreview = csb.createNode("shop/merchants_will_preview_baiying.csb")
	local root = csbHeroRecruitPreview:getChildByName("root")
	table.insert(self.roots, root)
    self:addChild(csbHeroRecruitPreview)
    self:onUpdateDraw()
	self:setContentSize(ccui.Helper:seekWidgetByName(root, "ScrollView_baiying_1"):getContentSize())
	
end

function HeroRecruitPreviewThree:init(types)
	self.types = types
end


function HeroRecruitPreviewThree:onExit()

end

function HeroRecruitPreviewThree:createCell()
	local cell = HeroRecruitPreviewThree:new()
