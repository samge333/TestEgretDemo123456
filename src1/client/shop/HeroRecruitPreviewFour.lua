...
HeroRecruitPreviewFour = class("HeroRecruitPreviewFourClass", Window)
    
function HeroRecruitPreviewFour:ctor()
    self.super:ctor()
	self.roots = {}
	self.types  = nil
end
...
function HeroRecruitPreviewFour:onHeadDraw(id)
	local csbItem = nil 
...
function HeroRecruitPreviewFour:onUpdateDraw()
	...
	local last = {}
	local lastAll = {}
	local arr1 = dms.searchs(dms["bounty_hero_param"], bounty_hero_param.bounty_group, 17)
	local arr2 = dms.searchs(dms["bounty_hero_param"], bounty_hero_param.bounty_group, 18)
	local arr3 = dms.searchs(dms["bounty_hero_param"], bounty_hero_param.bounty_group, 19)
	local arr4 = dms.searchs(dms["bounty_hero_param"], bounty_hero_param.bounty_group, 20)
	...
	-- [[绘制滚动层
	local m_ScrollView = ccui.Helper:seekWidgetByName(root, "ScrollView_heiqi_1")
...
function HeroRecruitPreviewFour:onEnterTransitionFinish()
	-- [[-------------------------------------
	-- add map for trial tower
    local csbHeroRecruitPreview = csb.createNode("shop/merchants_will_preview_heiqi.csb")
	local root = csbHeroRecruitPreview:getChildByName("root")
	table.insert(self.roots, root)
    self:addChild(csbHeroRecruitPreview)
    self:onUpdateDraw()
	
	self:setContentSize(ccui.Helper:seekWidgetByName(root, "ScrollView_heiqi_1"):getContentSize())
end

function HeroRecruitPreviewFour:init(types)
	self.types = types
end

function HeroRecruitPreviewFour:onExit()

end

function HeroRecruitPreviewFour:createCell()
	local cell = HeroRecruitPreviewFour:new()
