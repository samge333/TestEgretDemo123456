MagicCardMagicList = class("MagicCardMagicListClass", Window)

function MagicCardMagicList:ctor()
    self.super:ctor()
    self.roots = {}
    self.actions = {}
    ...
    local magic_card_magic_update_terminal = {
        _name = "magic_card_magic_update",
        ...
        _invoke = function(terminal, instance, params)
            if instance ~= nil and instance.roots ~= nil then
                instance:changeCardFormationInfo()
            end
			return true
        end,
        _terminal = nil,
    ...
    --需要删除列表中的卡片刷新
    local magic_card_magic_remove_card_update_terminal = {
        _name = "magic_card_magic_remove_card_update",
        _init = function (terminal)
        end,
        _inited = false,
        _instance = self,
    ...
    state_machine.add(magic_card_magic_update_terminal)
    state_machine.add(magic_card_magic_remove_card_update_terminal)    
    state_machine.init()
end

function MagicCardMagicList:init()
	return self
end

--删除消耗卡牌
function MagicCardMagicList:removeConsumeMagicCards()
...
--从列表中删除单个Item
function MagicCardMagicList:removeItemForId(removeId)
...
function MagicCardMagicList:onUpdate( dt )
...
function MagicCardMagicList:onUpdateDraw( ... )
   ...
   if _ED.user_magic_trap_info ~= nil then 
        ...
        for k,prop in pairs(_ED.user_magic_trap_info) do
            ...
            if cardType == 0  then 
                isEmpty = false
                if _ED.magicCard_formation[zstring.tonumber(prop.id)] == 1 then 
                    prop.isFormation = 1 
                else
                    prop.isFormation = 2 
                end
                table.insert(magics,prop)
            end
        end
        
        local function sortType( a, b )
            return tonumber(a.isFormation) < tonumber(b.isFormation)
        end
        table.sort(magics, sortType)
        for k,prop in pairs(magics) do
            local cell = MagicTrupCardCell:CreateCell():init(self.asyncIndex, 1, prop,false)
    ...
    self:changeCardFormationInfo()
    self.currentListView:jumpToTop()
end

function MagicCardMagicList:changeCardFormationInfo( ... )
    local root = self.roots[1]
    if root == nil then
        return
    end
    ccui.Helper:seekWidgetByName(root, "Panel_1"):setVisible(true)
    ccui.Helper:seekWidgetByName(root, "Panel_m"):setVisible(true) 
    local card_list = zstring.split("".._ED.magicCardInfo, ",")
    local count = 0
    local total = 0
    for k,v in pairs(card_list) do
    ...
    local Label_shuliang = ccui.Helper:seekWidgetByName(root, "Label_shuliang")
    Label_shuliang:setString(""..count .. "/" .. total)
    ...
    Label_shuliang1:setString("".. self.asyncIndex-1 .. "/" .. config[7])
end

function MagicCardMagicList:onUpdateDrawMagicListCounts()
    if self.currentListView ~= nil then 
        local itmes = self.currentListView:getItems()
        local Label_shuliang1 = ccui.Helper:seekWidgetByName(self.roots[1], "Label_5075")
        local config = zstring.split(dms.string(dms["pirates_config"], 6, pirates_config.param), ",")
        Label_shuliang1:setString("".. #itmes .. "/" .. config[7])     
    end
end

function MagicCardMagicList:onEnterTransitionFinish()
    local csbMagicListCard = csb.createNode("packs/magic_card_listview.csb")
    local root = csbMagicListCard:getChildByName("root")
    table.insert(self.roots, root)
    self:addChild(csbMagicListCard)
    self.currentListView = ccui.Helper:seekWidgetByName(root, "ListView_1")

    self:onUpdateDraw()
end

function MagicCardMagicList:onExit()
    state_machine.remove("magic_card_magic_update")
end
