...
local platform_request_for_payment_terminal = {
    ...
    _invoke = function(terminal, instance, params)
		...
		local goods_game_money = dms.string(dms["top_up_goods"], tonumber(m_reqcharge_info.rechargeIndex), top_up_goods.game_money)
		local goods_type = dms.int(dms["top_up_goods"], tonumber(m_reqcharge_info.rechargeIndex), top_up_goods.goods_type)
		local m_informUrlData = zstring.split(app.configJson.url,"iniServer")
		local m_informUrl = m_informUrlData[1].."pay/notifyAnfan.do"
		...
		if goods_type == 0 then
			typeName = _string_piece_info[207]
		else
			goods_game_money = dms.string(dms["top_up_goods"], tonumber(m_reqcharge_info.rechargeIndex), top_up_goods.money)
			typeName = _string_piece_info[217].."月卡"
		end
		handlePlatformRequest(0, PLATFORMRECHARGE, _ED.recharge_result.recharge_id .."|"..money.."|"..goods_name.."|"..goods_game_money.."|"..typeName.."|"..m_informUrl..
