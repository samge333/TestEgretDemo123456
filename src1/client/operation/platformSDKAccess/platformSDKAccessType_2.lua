...
local platform_manager_login_success_terminal = {
    ...
    _invoke = function(terminal, instance, params)
		local platformloginInfo = {
			...
			platform_account=params.platformInfo,		--游戏账号
			nickname="Guest",					--注册昵称
...
local platform_request_for_payment_terminal = {
    ...
    _invoke = function(terminal, instance, params)
        ...
        if app.configJson.OperatorName == "cayenne" then
            local AppStoreRecharge = cc.UserDefault:getInstance():getStringForKey("AppStoreRecharge")
            if AppStoreRecharge ~= nil and AppStoreRecharge ~= "" then
                local vzs = zstring.split(AppStoreRecharge, "|")
                local value = vzs[2]
                if _ED.selected_server ~= vzs[3] then
                    handlePlatformRequest(0, VERIFY_RECEIPT_SOUCCESS, ""..value)
                    fwin:close(fwin:find("ConnectingViewClass"))
                    return
                end
                local iDentity = tonumber(value)+1
                local specialpid = cc.UserDefault:getInstance():getStringForKey("paySpecialpid")
                ...
                if app.configJson.OperatorName == "cayenne" then
                    ...
                    --     if zstring.tonumber(specialpid) == 10 then
                    --         iDentity = 10
                    --     end
                    -- else
                ...
                local function responseGetServerListCallback(response)
                    if response.RESPONSE_SUCCESS and response.PROTOCOL_STATUS >= 0 then
                        handlePlatformRequest(0, VERIFY_RECEIPT_SOUCCESS, ""..value)
                        state_machine.excute("refresh_single_recharge_interface_list", 0, 0)
                        m_reqcharge_info.prevRechargeIndex = nil
                ...
                protocol_command.vali_top_up_order_number.param_list = ""..vzs[1].."\r\n"..iDentity.."\r\n".."AppStore".."\r\n"..vzs[4]
		...
		if app.configJson.OperatorName == "xiaomi" or app.configJson.OperatorName == "guopan-android" then
			fwin:open(ConnectingView:new(), fwin._windows)
		end
		...
		local money = dms.string(dms["top_up_goods"], tonumber(m_reqcharge_info.rechargeIndex), top_up_goods.money)
		local goods_name = dms.string(dms["top_up_goods"], tonumber(m_reqcharge_info.rechargeIndex), top_up_goods.goods_name)
		local game_money = dms.string(dms["top_up_goods"], tonumber(m_reqcharge_info.rechargeIndex), top_up_goods.game_money)
		...
		if app.configJson.OperatorName == "aladin" then
			local m_pay_type = dms.string(dms["top_up_goods"], tonumber(m_reqcharge_info.rechargeIndex), top_up_goods.pay_type)
			handlePlatformRequest(0, PLATFORMRECHARGE, _ED.recharge_result.recharge_id .."|"..money.."|"..goods_name..
			"|".._ED.user_info.user_gold.."|".._ED.vip_grade.."|".._ED.user_info.user_grade.."|".._ED.user_info.user_name..
			"|".._ED.user_info.user_id.."|".._ED.all_servers[_ED.selected_server].server_name.."|"..m_reqcharge_info.rechargeIndex..
			"|"..game_money.."|"..giveGold.."|"..goodsId.."|".._ED.all_servers[_ED.selected_server].server_number.."|"..zstring.tonumber(m_pay_type))
		elseif app.configJson.OperatorName == "nubia" then 	
			handlePlatformRequest(0, PLATFORMRECHARGE, _ED.recharge_result.recharge_id .."|"..money.."|"..goods_name..
			"|".._ED.user_info.user_gold.."|".._ED.vip_grade.."|".._ED.user_info.user_grade.."|".._ED.user_info.user_name..
			"|".._ED.user_info.user_id.."|".._ED.all_servers[_ED.selected_server].server_name.."|"..m_reqcharge_info.rechargeIndex..
			"|"..game_money.."|"..giveGold.."|"..goodsId.."|".._ED.all_servers[_ED.selected_server].server_number.."|".._ED.recharge_result.dataTimestamp.."|".._ED.recharge_result.nubiaSign)
		else
			handlePlatformRequest(0, PLATFORMRECHARGE, _ED.recharge_result.recharge_id .."|"..money.."|"..goods_name..
			"|".._ED.user_info.user_gold.."|".._ED.vip_grade.."|".._ED.user_info.user_grade.."|".._ED.user_info.user_name..
			"|".._ED.user_info.user_id.."|".._ED.all_servers[_ED.selected_server].server_name.."|"..m_reqcharge_info.rechargeIndex..
			"|"..game_money.."|"..giveGold.."|"..goodsId.."|".._ED.all_servers[_ED.selected_server].server_number)
		end
        return true
    end,
    _terminal = nil,
...
local platform_the_success_of_payment_terminal = {
    ...
    _invoke = function(terminal, instance, params)
        if app.configJson.OperatorName == "cayenne" then
            if cc.Application:getInstance():getTargetPlatform() == cc.PLATFORM_OS_IPHONE or cc.Application:getInstance():getTargetPlatform() == cc.PLATFORM_OS_IPAD then
                state_machine.excute("platform_the_success_of_payment_app", 0, {_rechargeCell = nil})
            else
                local function responseGetServerListCallback(response)
                    if response.RESPONSE_SUCCESS and response.PROTOCOL_STATUS >= 0 then
                        state_machine.excute("refresh_single_recharge_interface_list", 0, 0)
                ...
                _ED.recharge_result.recharge_id = _ED.recharge_result.recharge_id or m_reqcharge_info.prevRechargeIndex
                protocol_command.vali_top_up_order_number.param_list =  "".._ED.user_info.user_id.."\r\n".._ED.recharge_result.recharge_id.."\r\n"..app.configJson.OperatorName.."\r\n"..m_platform_app_ptext
                NetworkManager:register(protocol_command.vali_top_up_order_number.code, nil, nil, nil, nil, responseGetServerListCallback, false, nil)
            end
        else
    		if app.configJson.OperatorName == "aladin" then
    			protocol_command.vali_top_up_order_number.param_list =  "".._ED.user_info.user_id.."\r\n".._ED.recharge_result.recharge_id.."\r\n"..app.configJson.OperatorName.."\r\n"..m_platform_app_ptext
    			NetworkManager:register(protocol_command.vali_top_up_order_number.code, nil, nil, nil, nil, nil, false, nil)
    		end
    		
    		if app.configJson.OperatorName == "morefun" then
                m_tx_pay_index = 0
                m_tencent_pay_number = 20
    			m_tx_pay_start = true
    			return
    		end
    		--请求效验订单
...
--渠道接入状态机（支付成功app）
local platform_the_success_of_payment_app_terminal = {
    _name = "platform_the_success_of_payment_app",
    _init = function (terminal)
    end,
    _inited = false,
    _instance = nil,
    _state = 0,
    _invoke = function(terminal, instance, params)
        local rechargeCell = params._rechargeCell
        local value = m_platform_app_value
        local pText = m_platform_app_ptext
        local specialpid = cc.UserDefault:getInstance():getStringForKey("paySpecialpid")
        local function responseGetServerListCallback(response)
            if response.RESPONSE_SUCCESS and response.PROTOCOL_STATUS >= 0 then
                m_reqcharge_info.prevRechargeIndex = nil
        ...
        and cc.Application:getInstance():getTargetPlatform() ~= cc.PLATFORM_OS_MAC then
            if pText~="" then
                local met = "p"
                met = met..value
                local vzs = zstring.split(""..cc.UserDefault:getInstance():getStringForKey(met), "|")
                cc.UserDefault:getInstance():flush()
                if _ED.selected_server ~= vzs[3] then
                    handlePlatformRequest(0, VERIFY_RECEIPT_SOUCCESS, ""..value)
                    return
...
local platform_the_roles_tracking_terminal = {
    ...
    _state = 0,
    _invoke = function(terminal, instance, params)
		handlePlatformRequest(0, CC_GAME_DAYA_TRACKING, _ED.user_info.user_id.."|".._ED.user_info.user_name.."|".._ED.user_info.user_grade.."|".._ED.selected_server.."|".._ED.all_servers[_ED.selected_server].server_name.."|".._ED.user_info.user_gold.."|".._ED.vip_grade)
        return true
    end,
    _terminal = nil,
    _terminals = nil
...
local platform_the_role_establish_terminal = {
    ...
    _state = 0,
    _invoke = function(terminal, instance, params)
		handlePlatformRequest(0, CC_ROLE_CREATION, params.names.."|".._ED.selected_server.."|".._ED.all_servers[_ED.selected_server].server_name.."|".._ED.user_info.user_id.."|".._ED.user_info.user_name)
        return true
    end,
    _terminal = nil,
    _terminals = nil
...
local platform_facebook_request_friends_terminal = {
	_name = "platform_facebook_request_friends",
    _init = function (terminal)
    end,
    _inited = false,
    _instance = nil,
...
local platform_facebook_friends_request_terminal = {
	_name = "platform_facebook_friends_request",
    _init = function (terminal)
    end,
    _inited = false,
    _instance = nil,
    _state = 0,
    _invoke = function(terminal, instance, params)
		handlePlatformRequest(0, CC_FACEBOOK_FRIENDS_INVITE, _friends_text)
        return true
    end,
    _terminal = nil,
    _terminals = nil
...
local platform_facebook_bindaccount_request_terminal = {
	_name = "platform_facebook_bindaccount_request",
    _init = function (terminal)
    end,
    _inited = false,
    _instance = nil,
...
local platform_the_get_servers_info_terminal = {
	_name = "platform_the_get_servers_info",
    _init = function (terminal)
    end,
    _inited = false,
    _instance = nil,
    _state = 0,
    _invoke = function(terminal, instance, params)
		handlePlatformRequest(0, CC_GET_SERVERS_ID, params[1].."|"..params[2])
        return true
    end,
    _terminal = nil,
    _terminals = nil
...
local platform_operator_openurl_terminal = {
	_name = "platform_operator_openurl",
    _init = function (terminal)
    end,
    _inited = false,
    _instance = nil,
...
-- if __lua_project_id == __lua_project_warship_girl_a and _lua_release_language_param == _lua_release_language_zh then
...
state_machine.add(platform_the_success_of_payment_app_terminal)
...
state_machine.add(platform_facebook_request_friends_terminal)
state_machine.add(platform_facebook_friends_request_terminal)
state_machine.add(platform_facebook_bindaccount_request_terminal)
state_machine.add(platform_the_get_servers_info_terminal)
state_machine.add(platform_operator_openurl_terminal)
