...
local platform_manager_open_sdk_interface_terminal = {
    ...
    _state = 0,
    _invoke = function(terminal, instance, params)
		-- if app.configJson.UserAccountPlatformName == "tencent" then
			
		-- end
		if cc.UserDefault:getInstance():getStringForKey("tencentPay") == nil
		or cc.UserDefault:getInstance():getStringForKey("tencentPay") == "" then
			cc.UserDefault:getInstance():setStringForKey("tencentPay", "-1")
			cc.UserDefault:getInstance():flush()
		end
		if app.configJson.OperatorName ~= "gdtppk" then
			app.load("client.operation.platformSDKInterface.platform_SDK_login_tencent")
			fwin:open(platformSDKLoginTencent:new(), fwin._windows)
		end
        return true
    end,
    _terminal = nil,
...
local platform_manager_login_success_terminal = {
    ...
    _invoke = function(terminal, instance, params)
		...
		local loginInfo = zstring.split(params.platformAdditional, "/")
		m_platform_tx_login_info = ""
		m_platform_tx_login_info = loginInfo[1]
		if cc.UserDefault:getInstance():getStringForKey("tencentType") == "1" then
			m_platform_tx_login_info = m_platform_tx_login_info..","..loginInfo[2]
		else
			m_platform_tx_login_info = m_platform_tx_login_info..","..loginInfo[3]
		end
		m_platform_tx_login_info = m_platform_tx_login_info..","..loginInfo[3]
		m_platform_tx_login_info = m_platform_tx_login_info..","..loginInfo[4]
		m_platform_tx_login_info = m_platform_tx_login_info..","..loginInfo[5]
		m_platform_tx_login_info = m_platform_tx_login_info..","..loginInfo[6]
		m_platform_tx_login_info = m_platform_tx_login_info..","..loginInfo[7]
        return true
    end,
    _terminal = nil,
    _terminals = nil
...
local platform_request_for_payment_terminal = {
    ...
    _invoke = function(terminal, instance, params)
		...
		handlePlatformRequest(0, PLATFORMRECHARGE, _ED.recharge_result.recharge_id .."|"..money.."|"..goods_name.."|".._ED.user_info.user_gold.."|".._ED.user_info.capture_vip_grade.."|".._ED.user_info.user_grade.."|".._ED.user_info.user_name.."|".._ED.user_info.user_id.."|".._ED.all_servers[_ED.selected_server].server_name..
		"|"..game_money.."|"..giveGold.."|"..m_reqcharge_info.rechargeIndex)
        local rechargeIndex = 0
        if m_reqcharge_info.tencentRechargeIndex == nil 
            or m_reqcharge_info.tencentRechargeIndex == "" then
            rechargeIndex = m_reqcharge_info.rechargeIndex
        else
            rechargeIndex = m_reqcharge_info.tencentRechargeIndex
        end
		cc.UserDefault:getInstance():setStringForKey("tencentPay", rechargeIndex)
		cc.UserDefault:getInstance():flush()
        m_reqcharge_info.tencentRechargeIndex = nil
        return true
    end,
    _terminal = nil,
    _terminals = nil
...
local platform_the_success_of_payment_terminal = {
    ...
    _invoke = function(terminal, instance, params)
		...
		-- local loginInfo = zstring.split(m_platform_app_ptext, "/")
		-- m_platform_tx_login_info = ""
		-- m_platform_tx_login_info = loginInfo[1]
		-- m_platform_tx_login_info = m_platform_tx_login_info..","..loginInfo[2]
		-- m_platform_tx_login_info = m_platform_tx_login_info..","..loginInfo[3]
...
--渠道接入状态机（腾讯查询余额）
local tencent_balance_inquiry_terminal = {
    _name = "tencent_balance_inquiry",
    _init = function (terminal)
    end,
    _inited = false,
    _instance = nil,
    ...
    _invoke = function(terminal, instance, params)
		local function responseInitCallback(response)
			if response.RESPONSE_SUCCESS and response.PROTOCOL_STATUS >= 0 then
				if m_tx_pay_over == true then
					local function responseCallback(response)
						if response.RESPONSE_SUCCESS and response.PROTOCOL_STATUS >= 0 then
							handlePlatformRequest(0, CLEAR_RECEIPT, "")
							if state_machine.find("activity_pop_recharge_recharge_success") ~= nil then
								state_machine.excute("activity_pop_recharge_recharge_success", 0, nil)
							end
						end
					end	
					protocol_command.tencent_pay.param_list = cc.UserDefault:getInstance():getStringForKey("tencentPay")
					NetworkManager:register(protocol_command.tencent_pay.code, nil, nil, nil, nil, responseCallback, -1)
				end
			end
		end
		protocol_command.tencent_balance.param_list = cc.UserDefault:getInstance():getStringForKey("tencentPay")
		NetworkManager:register(protocol_command.tencent_balance.code, nil, nil, nil, nil, responseInitCallback, -1)
        return true
    end,
    _terminal = nil,
    _terminals = nil
...
state_machine.add(tencent_balance_inquiry_terminal)
