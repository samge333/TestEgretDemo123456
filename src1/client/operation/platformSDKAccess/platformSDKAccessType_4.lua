...
local platform_manager_login_success_terminal = {
    ...
    _state = 0,
    _invoke = function(terminal, instance, params)
		if params.platformInfo=="" or params.platformInfo==nil then
			state_machine.excute("platform_manager_login_lose", 0, "platform_manager_login_lose.")
			return
		end
		
		if app.configJson.OperatorName ~= "ggame" then
...
--渠道接入状态机（获取支付信息）
local platform_get_pay_info_terminal = {
    _name = "platform_get_pay_info",
    _init = function (terminal)
    end,
    _inited = false,
    _instance = nil,
    _state = 0,
    _invoke = function(terminal, instance, params)
		m_get_pay_info = m_get_pay_info+1
		if m_get_pay_info == 1 then
			local function responseGetServerListCallback(response)
				if response.RESPONSE_SUCCESS and response.PROTOCOL_STATUS >= 0 then
					handlePlatformRequest(0, CC_GET_PAY_INFO, _ED.recharge_result.recharge_id.."_"..m_sOperatorName.."|".._ED.user_info.user_id.."|".._ED.user_info.user_name.."|".._ED.selected_server)
				end
			end
			protocol_command.request_top_up_order_number.param_list = "1".."\r\n"..m_sPayMould
			NetworkManager:register(protocol_command.request_top_up_order_number.code, nil, nil, nil, nil, responseGetServerListCallback, false, nil)
		end
        return true
    end,
    _terminal = nil,
...
--渠道接入状态机（获取支付信息完结）
local platform_get_pay_info_over_terminal = {
    _name = "platform_get_pay_info_over",
    _init = function (terminal)
    end,
    _inited = false,
    _instance = nil,
...
local platform_request_for_payment_terminal = {
    ...
    _invoke = function(terminal, instance, params)
		if __lua_project_id == __lua_project_red_alert then
			if zstring.tonumber(m_reqcharge_info.rechargeIndex)-1 == 3 then
				cc.UserDefault:getInstance():setStringForKey("paySpecialpid", "-1")
				cc.UserDefault:getInstance():flush()
			end
		end
		if app.configJson.OperatorName ~= "play800-android" and app.configJson.OperatorName ~= "play800-enterprise" 
		and app.configJson.OperatorName ~= "mobilemm" and app.configJson.OperatorName ~= "play800-android-ggame" 
		and app.configJson.OperatorName ~= "play800-android-ggame2" and app.configJson.OperatorName ~= "play800-android-ggame3" 
		and app.configJson.OperatorName ~= "play800-android-ggame4" then
			--fwin:open(ConnectingView:new(), fwin._windows)
		end
		...
		if (app.configJson.OperatorName == "play800" and app.configJson.PayOperatorName ~= "app") or app.configJson.OperatorName == "play800-android" 
			or (app.configJson.OperatorName == "play800-2" and app.configJson.PayOperatorName ~= "app") 
			or app.configJson.OperatorName == "play800-enterprise" or app.configJson.OperatorName == "play800-enterprise-two" 
			or app.configJson.OperatorName == "play800-ios" or app.configJson.OperatorName == "sina" 
			or app.configJson.OperatorName == "baidu-sem" or app.configJson.OperatorName == "appduoduo" 
			or app.configJson.OperatorName == "ptbus" or app.configJson.OperatorName == "mobilemm" 
			or app.configJson.OperatorName == "play800-android-ggame" or app.configJson.OperatorName == "play800-android-ggame2" 
			or app.configJson.OperatorName == "play800-android-ggame3" or app.configJson.OperatorName == "play800-android-ggame4" 
			or app.configJson.OperatorName == "ggame" then 
			handlePlatformRequest(0, PLATFORMRECHARGE, _ED.recharge_result.recharge_id.."_"..m_sOperationPlatformName.."|".._ED.user_info.user_id.."|".._ED.user_info.user_name.."|".._ED.selected_server.."|"..money..
		"|"..game_money.."|"..giveGold.."|"..m_reqcharge_info.rechargeIndex.."|"..goods_name.."|".._ED.all_servers[_ED.selected_server].server_number)
		else
			local indexs = tonumber(m_reqcharge_info.rechargeIndex)-1
			if __lua_project_id == __lua_project_red_alert then
				if indexs == 9 then
					cc.UserDefault:getInstance():setStringForKey("paySpecialpid", ""..indexs)
					cc.UserDefault:getInstance():flush()
					indexs = 3
				end
			end
			handlePlatformRequest(0, PLATFORMRECHARGE, _ED.recharge_result.recharge_id.."|"..indexs.."|"..money.."|"..game_money.."|"..giveGold)
		end
        return true
    end,
    _terminal = nil,
		...
		if __lua_project_id == __lua_project_red_alert then
			if cc.Application:getInstance():getTargetPlatform() == cc.PLATFORM_OS_IPHONE or cc.Application:getInstance():getTargetPlatform() == cc.PLATFORM_OS_IPAD then
				if app.configJson.PayOperatorName == "app" then
					if zstring.tonumber(m_sVersion)*1000 >= 1523 and zstring.tonumber(m_sVersion)*1000 <= 1525 then
						return
					end
				end
			end
    ...
    _invoke = function(terminal, instance, params)
		...
		local function responseGetServerListCallback(response)
			...
			if response.RESPONSE_SUCCESS and response.PROTOCOL_STATUS >= 0 then
				...
				app.load("client.red_alert.activity.wonderful.ActivityPopRecharge")
				if __lua_project_id == __lua_project_red_alert then
					if zstring.tonumber(specialpid) == 9 then
						TipDlg.drawTextDailog(_string_piece_info[427])
					end
                end
				state_machine.excute("activity_pop_recharge_recharge_success", 0, nil)
...
local platform_the_roles_tracking_terminal = {
    ...
    _state = 0,
    _invoke = function(terminal, instance, params)
		handlePlatformRequest(0, CC_GAME_DAYA_TRACKING, _ED.user_info.user_id.."|".._ED.user_info.user_name.."|".._ED.user_info.user_grade.."|".._ED.selected_server.."|".._ED.all_servers[_ED.selected_server].server_name.."|".._ED.user_info.user_gold)
        return true
    end,
    _terminal = nil,
    _terminals = nil
...
state_machine.add(platform_get_pay_info_terminal)
state_machine.add(platform_get_pay_info_over_terminal)
