...
--材料的仓库
-----------------------------
SmMaterialWarehouse = class("SmMaterialWarehouseClass", Window)

local sm_material_warehouse_window_open_terminal = {
    _name = "sm_material_warehouse_window_open",
    ...
    _invoke = function(terminal, instance, params)
		local panel = SmMaterialWarehouse:new():init(params)
		fwin:open(panel)
        return panel
    end,
    _terminal = nil,
...
state_machine.add(sm_material_warehouse_window_open_terminal)
state_machine.init()

function SmMaterialWarehouse:ctor()
    self.super:ctor()
    self.roots = {}
    ...
    local function init_sm_material_warehouse_terminal()
		--显示界面
		local sm_material_warehouse_show_terminal = {
            _name = "sm_material_warehouse_show",
            _init = function (terminal) 
		...
		--隐藏界面
		local sm_material_warehouse_hide_terminal = {
            _name = "sm_material_warehouse_hide",
            _init = function (terminal) 
        ...
        local sm_material_warehouse_update_draw_terminal = {
            _name = "sm_material_warehouse_update_draw",
            _init = function (terminal) 
		...
		state_machine.add(sm_material_warehouse_show_terminal)	
		state_machine.add(sm_material_warehouse_hide_terminal)
        state_machine.add(sm_material_warehouse_update_draw_terminal)

        state_machine.init()
    end
    init_sm_material_warehouse_terminal()
end

function SmMaterialWarehouse:onHide()
...
function SmMaterialWarehouse:onShow()
    self:setVisible(true)
    self:onUpdateDraw()
end

function SmMaterialWarehouse:getSortedHeroes()
    ...
    for i, prop in pairs(_ED.user_prop) do
        ...
        if dms.atoi(propData, prop_mould.storage_page_index) == 17 then
...
function SmMaterialWarehouse:getSortedEquips()
    local tSortedHeroes = {}
...
function SmMaterialWarehouse:onUpdateDraw()
    self.prop_list = {}
...
function SmMaterialWarehouse:onUpdate(dt)
...
function SmMaterialWarehouse:onEnterTransitionFinish()

end

function SmMaterialWarehouse:onInit( )
...
function SmMaterialWarehouse:init(params)
...
function SmMaterialWarehouse:onExit()
    state_machine.remove("sm_material_warehouse_show")    
    state_machine.remove("sm_material_warehouse_hide")
    state_machine.remove("sm_material_warehouse_update_draw")
end
