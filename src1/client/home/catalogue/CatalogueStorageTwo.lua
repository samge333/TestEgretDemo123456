...
-- 最后修改人：
-------------------------------------------------------------------------------------------------------
CatalogueStorageTwo = class("CatalogueStorageTwoClass", Window)
	
function CatalogueStorageTwo:ctor()
    self.super:ctor()
    self.roots = {}
	self.all = {}
	self.allHero = {}
...
function CatalogueStorageTwo.loading(texture)
	local myListView = CatalogueStorageTwo.myListView
	if myListView ~= nil then
		...
		if __lua_project_id == __lua_project_digimon_adventure or __lua_project_id == __lua_project_naruto 
			...
			cell:init(CatalogueStorageTwo.dTreasureArray[CatalogueStorageTwo.asyncIndex], 4)
		else
			cell = CatalogueStorageList:createCell()
			cell:init(CatalogueStorageTwo.dTreasureArray[CatalogueStorageTwo.asyncIndex])
		end
		
		myListView:addChild(cell)
		CatalogueStorageTwo.asyncIndex = CatalogueStorageTwo.asyncIndex + 1
...
function CatalogueStorageTwo:onUpdateDraw()
	local root = self.roots[1]
	-- camp_preference		阵营
	local listView = ccui.Helper:seekWidgetByName(root, "ListView_02")
	local loadBar = ccui.Helper:seekWidgetByName(root, "LoadingBar_2")
	local Text_2 = ccui.Helper:seekWidgetByName(root, "Text_12")
	...
	local temp = 0
	if __lua_project_id == __lua_project_gragon_tiger_gate or __lua_project_id == __lua_project_l_digital or __lua_project_id == __lua_project_l_pokemon or __lua_project_id == __lua_project_l_naruto  or __lua_project_id == __lua_project_red_alert then
		data = _ED.all_ship_mould[4]	
	else
		data = dms.searchs(dms["ship_mould"], ship_mould.camp_preference, 4)
	end
	...
	CatalogueStorageTwo.myListView = listView
	CatalogueStorageTwo.dTreasureArray = self.all
	CatalogueStorageTwo.asyncIndex = 1
	CatalogueStorageTwo.myListView:removeAllItems()
	...
	-- end
	Text_2:setString(_string_piece_info[296])
	local expCount = (kNum)/(num)*100
	loadBar:setPercent(expCount)
end

function CatalogueStorageTwo:onEnterTransitionFinish()
	-- local csbCatalogueStorageTwo = csb.createNode("system/Atlas_listview_2.csb")

	-- self:addChild(csbCatalogueStorageTwo)
	
	-- local root = csbCatalogueStorageTwo:getChildByName("root")
	-- table.insert(self.roots, root)
	-- self:onUpdateDraw()
   
end
function CatalogueStorageTwo:onInit()
	local csbCatalogueStorageTwo = csb.createNode("system/Atlas_listview_2.csb")
...
function CatalogueStorageTwo:init()
	self:onInit()
	
end

function CatalogueStorageTwo:onExit()
	-- if fwin:find("CatalogueStorageTwoClass") ~= nil then
	CatalogueStorageTwo.myListView:removeAllItems()
		CatalogueStorageTwo.myListView = nil
		CatalogueStorageTwo.asyncIndex = 1
	-- end
end